...
from utils.constants import TIMES, set_appbar
...
def init_clicker(page: ft.Page):
    ...
    set_appbar(page, 'clicker', get_items_for_appbar)
    page.update()
    ...
    def reset(e=None):
        ...
        timer.reset(Time, end)

    count_text = ft.Text(
        '0', size=50, weight=ft.FontWeight.BOLD, text_align=ft.TextAlign.END, width=100
    )
    timer_text = ft.Text('')

    progress_bar = ft.ProgressBar(width=600, height=10, value=1)
    start_button = ft.FilledButton(
        'Покажи на что способен)', width=370, height=100, on_click=start_func
    )
    ...
    record_text = ft.Text(
        f'Твой рекорд: {rec_cps if rec_cps != "None" else "0"} среднего кпс', size=30
    )
    ...
    if wwsettings.getSetting('linechart'):
        chart = ft.LineChart(
            ...
            bottom_axis=ft.ChartAxis(
                labels=[
                    ft.ChartAxisLabel(value=1, label=ft.Text('1')),
